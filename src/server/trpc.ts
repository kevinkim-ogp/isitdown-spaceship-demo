...
const loggerWithVersionMiddleware = loggerMiddleware.unstable_pipe(
  async ({ next, ctx }) => {
    ...
    const clientVersion = req.headers.get(APP_VERSION_HEADER_KEY.toLowerCase())
    ...
    // Only set response header if res is available (not needed in Next.js App Router)
    if (res) {
      res.headers.set(APP_VERSION_HEADER_KEY, serverVersion)
    }
...
const contentTypeHeaderMiddleware = t.middleware(async ({ ctx, next }) => {
  if (
    ctx.req.body &&
    !ctx.req.headers.get('content-type')?.startsWith('application/json')
  ) {
    throw new TRPCError({
      code: 'BAD_REQUEST',
...
const authMiddleware = t.middleware(async ({ next, ctx }) => {
  ...
  // Transform to match the expected user shape
  const user: DefaultMeSelect = {
